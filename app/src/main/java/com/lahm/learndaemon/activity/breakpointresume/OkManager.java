...
public class OkManager {
    ...
    public OkManager(String path, Context context, DownLoad.IProgress progress) {
        this.path = path;
        this.context=context;
        this.progress = progress;
        this.handler =  new Handler();
        this.name = path.substring(path.lastIndexOf(".")+1);
        rootFile = FileUtils.getRootFile(context);
    ...
    private void downLoadFile() {
        try {
            if (file == null){
                file = new File(rootFile,"test."+name);
                raf = new RandomAccessFile(file.getPath(),"rwd");
